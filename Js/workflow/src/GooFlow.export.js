...
if(typeof GooFlow.prototype.exportDiagram!=='function'){
	var Cmder = {
		//构建背景
		...
		_analyseLabel: function(labelDom){
			...
			var transform;
			if(tagName==='TD'){
				var div=labelDom.children("div")[0];
				offsetLeft += div.offsetLeft-this._toNum($(div).css("margin-left"));
				offsetTop += div.offsetTop-this._toNum($(div).css("margin-top"));
				transform = labelDom.children("div").css("transform");
			}else{
				transform = labelDom.css("transform");
			}

			var lineHeight = this._toNum(labelDom.css("line-height"));
			var fontSize = labelDom.css('font-size');
			return {
				font: '14px '+labelDom.css('font-family'),
		...
		_analyseIcon: function(iconDom){
			...
			if(bgImg && bgImg!=='none'){//CSS SPITE背景图式
				...
				$.extend(property,{
					font: '18px '+iconDom.css('font-family').split(' ')[0],//图标矢量字体库
		...
		_analyseNode : function(nodeDom){
			var spClass="";
			if(nodeDom.hasClass("item_ellipse")){
				spClass="ellipse";
			}
			if(nodeDom.hasClass("item_rhomb")){
				spClass="rhomb";
			}
			if(nodeDom.hasClass("item_round")){
				spClass="round";
			}
			if(nodeDom.hasClass("item_parallelogram")){
				spClass="parallelogram";
			}
			var property={
			...
			if(nodeDom.hasClass("item_round")||nodeDom.hasClass("item_rhomb")){
		...
		_fillIcon:function(canvas, bgLeft,bgTop,icon, bgImage,scale){
			var ctx = canvas.getContext('2d');
			if(icon.backgroundImage===undefined){//矢量字体图标
				...
				var X=bgLeft+icon.offsetLeft+4+icon.width/2, Y=bgTop+icon.offsetTop+2+icon.height/2;
				ctx.save();
				ctx.translate(X,Y);
				ctx.scale(scale,scale);
				//  绘制内容
				ctx.fillText(icon.content, 0, 0);
				ctx.restore();
				return null;
				...
				ctx.drawImage(bgImage, icon.pX, icon.pY, icon.width*scale, icon.height*scale,
					bgLeft+icon.offsetLeft+4+tmpX, bgTop+icon.offsetTop+4+tmpY, icon.width*scale, icon.height*scale );
			}
		},
		_fillLabel: function(canvas, bgLeft, bgTop, label,scale, rhomb) {
			var ctx = canvas.getContext('2d');
			...
			for(var i=0;i<str.length;i++){
				...
				if(i!==str.length-1){
					if( lineWidth  >= label.width/scale ){
		...
		renderAreas:function(canvas,areas, iconBgImage,scale){
			var ctx = canvas.getContext('2d');
			for(var key in areas) {
				...
				ctx.fill();
				ctx.stroke();
				this._fillIcon(canvas, area.left-3, area.top-3, area.icon, iconBgImage,scale);
				this._fillLabel(canvas, area.left, area.top, area.label,scale);
			}
		},

		//根据节点信息在背景上画一组节点（传参nodes为要绘制的节点详细json信息）
		renderNodes:function(canvas,nodes,iconBgImage,scale){
			var ctx = canvas.getContext('2d');
			var imgLoadFuncs=[];
			for(var key in nodes){
				...
				if(node.spClass==='ellipse') {
					ctx.BezierEllipse(node.left + node.width / 2, node.top + node.height / 2, node.width / 2, node.height / 2).fill();
				}else if(node.spClass==='rhomb'){
					ctx.save();
					ctx.translate(node.left+node.width/2, node.top+node.height/2);
					ctx.rotate(45*Math.PI/180);
					ctx.roundRect(-node.width/2,-node.height/2, node.width-node.borderWidth, node.height-node.borderWidth, node.borderRadius).fill();
					ctx.restore();
				}else if(node.spClass==='parallelogram'){
					ctx.save();
					ctx.translate(node.left+node.width/2, node.top+node.height/2);
					ctx.transform(1,0,-(12/node.height),1,0,0);
					ctx.roundRect(-node.width/2,-node.height/2, node.width-node.borderWidth, node.height-node.borderWidth, node.borderRadius).fill();
					ctx.restore();
				}else{
				...
				ctx.stroke();
				this._fillIcon(canvas, node.left+(node.borderRadius>6? -3:0), node.top, node.icon, iconBgImage,scale);
				this._fillLabel(canvas, node.left, node.top, node.label,scale,
					(node.spClass==='rhomb'? {x:node.width/3*1.41,y:node.height/4}:undefined)
				);
			}
			return imgLoadFuncs;
		},
...
// 		ctx.stroke();
// 	};
	CanvasRenderingContext2D.prototype.BezierEllipse=function(x, y, a, b){
		var k = .5522848,
			ox = a * k, // 水平控制点偏移量
			oy = b * k; // 垂直控制点偏移量
	...
	GooFlow.prototype.exportDiagram=function(fileName) {
		...
		var tempFunc=function(){
			Cmder.renderAreas(canvas,areaEl,iconImage,Goo.$scale);
			Cmder.renderNodes(canvas,nodeEl,iconImage,Goo.$scale);

			//处理连线
			var ctx = canvas.getContext('2d');//取得画布的2d绘图上下文
			ctx.restore();
			if(window.ActiveXObject || "ActiveXObject" in window || navigator.userAgent.indexOf("Edge")>-1){//当为IE11及以下版本浏览器时
				var lineEl={};
...
if(typeof GooFlow.prototype.exportDataFile!=='function'){
	/**
	 * 导出JSON结构的数据，到本地文件
	 * @param {string} content  待导出的内容，字符串
	 * @param {string} fileName 文件名
	 */
	function exportFileStream(content, fileName, fileType) {
		...
		try{
			...
			oPop.document.write('<html><head><title>'+fileName+"."+fileType+'</title></head><body><pre>'
				+(fileType==='xml'? content.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'):content)
				+'</pre></body></html>');
		}
	}
	GooFlow.prototype.exportDataFile=function(fileName, fileType){
		if(fileType==='json'){
			exportFileStream(JSON.stringify(this.exportData()),fileName,'json');
		}else if(fileType==='xml'){
			exportFileStream(this.exportBpmnXML(),fileName,'xml');
		}else	return;
	}
