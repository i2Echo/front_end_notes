...
(function (global, factory) {
	...
	var GooFlow = function (selector, property) {
		...
		//绑定右键事件:右击空白处时，blurItem
		this.$workArea.on("contextmenu", {
			inthis: this
		}, function (e) {
			var tar = $(e.target);
			console.log(tar[0].tagName);
			if (tar[0].tagName.toUpperCase() === 'SVG' || tar.hasClass("GooFlow_work_vml")) {
	...
	GooFlow.prototype = {
			...
			this.$group.mouseup({
				...
				if (e.data.inthis.$ghost.css("display") === "none") {
					...
					e.data.inthis.addArea(new Date().getTime(), {
						...
						left: X / This.$scale,
						top: Y / This.$scale,
						color: color[e.data.inthis.$max % 5],
						width: 200,
						height: 100
					});
					e.data.inthis.$max++;
		...
		_initWorkForNode: function () {
			...
			this.$workArea.on("mousedown", ".GooFlow_item", {
				...
				var line = GooFlow.prototype.drawLine("GooFlow_tmp_line", [X, Y], [X, Y], {
					mark: true,
					dash: true
				}, 1);
		...
		_initLinePointsChg: function () {
			this.$mpFrom.on("mousedown", {
				...
				var line = GooFlow.prototype.drawLine("GooFlow_tmp_line", [ps[0], ps[1]], [pe[0], pe[1]], {
					mark: true,
					dash: true
				}, 1);
				This.$draw.appendChild(line);
		...
		getItemInfo: function (id, type) {
			switch (type) {
				case "node":
					return JSON.parse(JSON.stringify(this.$nodeData[id] || null));
				case "line":
					return JSON.parse(JSON.stringify(this.$lineData[id] || null));
				case "area":
					return JSON.parse(JSON.stringify(this.$areaData[id] || null));
				default:
					return JSON.parse(JSON.stringify(this.$nodeData[id] || this.$lineData[id] || this.$areaData[id] || null));
			}
		},
		//快捷设置某个单元的自定义扩展属性，用户可用其设置只与具体业务相关的独有属性
		setItemExtra: function (id, type, props) {
			if (id == null || id === '') return;
			if (type == null || type === '') return;
			var obj = type === 'node' ? this.$nodeData[id] : type === 'line' ? this.$lineData[id] : this.$areaData[id];
			for (var key in props) {
				obj[key] = props[key];
			}
		},
		//快捷批量删除某个单元的自定义扩展属性，用户可用其删除只与具体业务相关的独有属性
		delItemExtra: function (id, type, propKeys) {
			if (id == null || id === '') return;
			if (type == null || type === '') return;
			var obj = type === 'node' ? this.$nodeData[id] : type === 'line' ? this.$lineData[id] : this.$areaData[id];
			for (var i = 0; i < propKeys.length; ++i) {
				delete obj[propKeys[i]];
			}
		},
		...
		addNode: function (id, json) {
			...
			var status = json.displayStatus ? json.displayStatus : ''
			if (json.type.indexOf(" round") > -1) {
				json.width = 26;
				json.height = 26;
				this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark +' '+ status + "' id='" + id + "' style='top:" + json.top * this.$scale + "px;left:" + json.left * this.$scale + "px'><table cellspacing='0' style='width:" + (json.width * this.$scale - 2) + "px;height:" + (json.height * this.$scale - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
			} else if (json.type.indexOf(" rhomb") > 0) {
				json.width = 30;
				json.height = 30;
				this.$nodeDom[id] = $("<div class='GooFlow_item item_rhomb" + mark +' '+ status + "' id='" + id + "' style='top:" + json.top * this.$scale + "px;left:" + json.left * this.$scale + "px'><table cellspacing='0' style='width:" + (json.width * this.$scale - 2) + "px;height:" + (json.height * this.$scale - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
			} else {
			...
			if (json.resultRecords) {
				var nodeEle = this.$nodeDom[id];
				var strList = '';
				for(var i=0; i<json.resultRecords.length; i++) {
					var str = `<div style="margin-top: 10px;padding: 10px;border-radius: 3px;background: ${(json.resultRecords[i].result == 1 || json.resultRecords[i].result == 41) ? '#B2DFDB' : '#FFE0B2'}">
											<div><span>处理人: </span>${json.resultRecords[i].processName || '暂无数据'} </div>
											<div><span>处理时间: </span>${json.resultRecords[i].date || '暂无数据'} </div>
											<div><span>处理记录: </span>${json.resultRecords[i].resultComment || '暂无数据'} </div>
										</div>`

					strList += str;
				}
				var $layer = $(`<div class="layer-record" style="
															position: absolute;
															border-radius: 4px;
															left: 0;
															top: 0;
															...
															box-shadow: 0 5px 5px 0 rgba(0,0,0,.14), 0 3px 1px -2px rgba(0,0,0,.2), 0 1px 5px 0 rgba(0,0,0,.12);">
																<h3 style="font-weight: 700;color:${json.displayStatus == 'active' ? '#FF9800' : '#00BFA5'};">
																${json.displayStatus == 'active' ? '当前节点待处理' : '当前节点已处理'}</h3>
																${strList}
															</div>`)
	
				$('.GooFlow').append($layer)
				var timer = null;
				nodeEle.mouseenter(function(){
					clearTimeout(timer)
					timer = null
					var offset = nodeEle.offset();
					var width = nodeEle.outerWidth()
					var height = nodeEle.outerHeight()
					var r = parseInt(nodeEle.css('right'))
					var top = offset.top - ~~(height/2)
					var left = offset.left + width
					console.log(offset.left, $layer.outerWidth())
					if(r < $layer.outerWidth()){
						left = offset.left - $layer.outerWidth()
					}
					$layer.css({
						'top': ~~top+'px',
						'left': ~~(left-5)+'px'
		...
		//设置某个单元的自定义颜色
		setItemColor: function (id, type, color) {
			if (id == null || id === '') return;
			if (type == null || type === '') return;
			if (type === 'node') {
				this.$nodeData[id].color = color;
				this.$nodeDom[id].css("background-color", color);
				if (!this.$nodeData[id].marked) {
					this.$nodeDom[id].css("border-color", color);
				}
			} else if (type === 'line') {
				this.$lineData[id].color = color;
				if (!this.$lineData[id].marked) {
					if (GooFlow.prototype.useSVG !== "") {
						this.$lineDom[id].childNodes[1].setAttribute("stroke", color);
					} else {
						this.$lineDom[id].strokeColor = color;
					}
				}
			}
		},
		//设置某个单元的自字义文字颜色
		setItemFontColor: function (id, type, color) {
			if (id == null || id === '') return;
			if (type == null || type === '') return;
			if (type === 'node') {
				this.$nodeData[id].fontColor = color;
				this.$nodeDom[id].find(".span").css("color", color);
				this.$nodeDom[id].find("td:eq(1)").css("color", color);
			} else if (type === 'line') {
				this.$lineData[id].fontColor = color;
				if (GooFlow.prototype.useSVG !== "") {
					this.$lineDom[id].childNodes[2].setAttribute("fill", color);
				} else {
					this.$lineDom[id].childNodes[1].style.color = color;
				}
			}
		},
		...
		_suitSize: function () {
			...
			return {
				width: maxW * this.$scale,
				height: maxH * this.$scale
			}
		},
		//载入一组数据
		...
		addLineDom: function (id, lineData) {
			var n1 = this.$nodeData[lineData.from],
				n2 = this.$nodeData[lineData.to]; //获取开始/结束节点的数据
			if (!n1 || !n2) return;
			//开始计算线端点坐标
			var res;
		...
		addLine: function (id, json) {
			...
			var n1 = this.$nodeData[json.from],
				n2 = this.$nodeData[json.to]; //获取开始/结束节点的数据
			if (!n1 || !n2) return;
			//避免两个节点间不能有一条以上同向接连线
			for (var k in this.$lineData) {
		...
		markItem: function (id, type, mark) {
			if (type === "node") {
				...
				if (mark) {
					...
					if (id !== this.$focus) this.$nodeDom[id].css("border-color", data.color || GooFlow.color.node);
				}
